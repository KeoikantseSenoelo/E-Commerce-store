// Remove any repeated React imports or unnecessary imports above.
...
const BagPage = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row p-4 bg-gray-100 min-h-screen">
      ...
      <aside className="hidden lg:block w-64 bg-white p-4 shadow-md">
        {/* Sidebar content (e.g., Bag Sidebar) can go here */}
      </aside>

      {/* Main content - This will adjust on mobile view */}
      <main className="flex-1 p-8 bg-gray-100 w-full lg:w-[calc(100%-256px)]">
          ...
          <div className="space-y-4">
            {cartItems.map((item, index) => (
              item && item.id ? (  // Check if the item and its id exist
                ...
                <div key={index} className="bg-white p-4 shadow rounded-lg w-full sm:w-3/4 md:w-3/4 xl:w-2/3 h-auto min-h-48">
